...
const setCharacter = (
  ...
  // Touch variables to satisfy TS compiler unused check
  if (renderer && scene) { /* no-op */ }
  ...
  const loadCharacter = () => {
    return new Promise<GLTF | null>(async (resolve, reject) => {
      try {
        ...
        const fallbackUrl = "/models/character.glb";
        ...
        if (isSecure && !isMobile) {
          try {
            ...
            primaryUrl = URL.createObjectURL(new Blob([encryptedBlob]));
          } catch (decryptError) {
            console.warn("Decryption failed, falling back directly to unencrypted model:", decryptError);
        ...
        const performLoad = (url: string, isFallbackAttempt: boolean) => {
          loader.load(
            url,
            (gltf) => {
            ...
            (error) => {
              ...
              if (!isFallbackAttempt && url !== fallbackUrl) {
                console.warn("Retrying character load using fallback unencrypted URL...");
                performLoad(fallbackUrl, true);
              } else {
                reject(error);
              }
            }
